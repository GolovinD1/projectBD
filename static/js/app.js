...
async function loadGames() {
    try {
        const { data: games, error } = await supabaseClient
            .from('games')
            .select('id, title, developer, publisher, release_year, description, genre_id, genres(name)');
...
async function loadMyGames() {
    ...
    try {
        ...
        const formattedGames = userGames.map(item => ({
            ...
            genre: item.games?.genres?.name || null
        }));
...
function displayGames(games, containerId) {
    ...
    games.forEach(game => {
        ...
        gameCard.innerHTML = `
            ...
            <span class="genre">${game.genre || 'Без жанра'}</span>
            <div class="actions">
...
function displayMyGames(games) {
    ...
    games.forEach(game => {
        ...
        gameCard.innerHTML = `
            <h3>${game.title}</h3>
            <p><strong>Разработчик:</strong> ${game.developer || 'Не указан'}</p>
            <p><strong>Жанр:</strong> ${game.genre || 'Без жанра'}</p>
...
async function loadStatistics() {
    ...
    try {
        ...
        userGames.forEach(item => {
            const genreName = item.games?.genres?.name || 'Без жанра';
...
async function loadRecommendations() {
    ...
    try {
        ...
        const genreCounts = {};
        userGames.forEach(item => {
            const genreId = item.games?.genre_id;
            if (genreId) {
        ...
        // Фильтруем игры, которые уже в коллекции
